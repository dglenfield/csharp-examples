...
switch (key)
{
    ...
    case ConsoleKey.D2 or ConsoleKey.NumPad2:
        builder.DataSource = "tcp:apps-services-book-dg.database.windows.net,1433";
        break;
...
WriteLine("| {0,5} | {1,-35} | {2,10} |", "Id", "Name", "Price");
WriteLine(horizontalLine);
...
await using (FileStream jsonStream = File.Create(jsonPath))
{
    ...
    while (await reader.ReadAsync())
    {
        ...
        WriteLine("| {0,5} | {1,-35} | {2,10:C} |",
            reader.GetFieldValueAsync<int>("ProductId").Result,
            reader.GetFieldValueAsync<string>("ProductName").Result,
            reader.GetFieldValueAsync<decimal>("UnitPrice").Result);
