...
public class HomeController : Controller
{
    ...
    public async Task<IActionResult> Index(string id = "1")
    {
        ...
        try
        {
            HttpClient client = _clientFactory.CreateClient("Northwind.GraphQL.Service");
            ...
            if (!response.IsSuccessStatusCode) 
            {
                ...
                model.Errors = [new Error { Message = "Service is not successfully responding to GET requests." }];
                return View(model);
            }
        ...
        catch (Exception ex)
        {
            ...
            model.Errors = [new Error { Message = ex.Message }];
        }
