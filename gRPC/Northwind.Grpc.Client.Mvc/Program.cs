using Grpc.Core; // To use StatusCode.
using Grpc.Net.Client.Configuration;
using Northwind.Grpc.Client.Mvc;
using Northwind.Grpc.Client.Mvc.Interceptors; // To use Greeter.GreeterClient.
...
builder.Services.AddGrpcClient<Shipper.ShipperClient>("Shipper", options =>
{
    options.Address = new Uri("https://localhost:5131");
});

builder.Services.AddGrpcClient<Product.ProductClient>("Product", options =>
{
    options.Address = new Uri("https://localhost:5131");
}).AddInterceptor<ClientLoggingInterceptor>();

builder.Services.AddGrpcClient<Employee.EmployeeClient>("Employee", options =>
{
    options.Address = new Uri("https://localhost:5131");
});
