...
public class HomeController : Controller
{
    ...
    public async Task<IActionResult> Index(string name = "Henrietta", int id = 1)
    {
        ...
        try
        {
            ...
            AsyncUnaryCall<ShipperReply> shipperCall = _shipperClient.GetShipperAsync(new ShipperRequest { ShipperId = id },
                deadline: DateTime.UtcNow.AddSeconds(3)); // Deadline must be a UTC DateTime.
            ...
            model.ShipperSummary = "Shipper from gRPC service: " +
                $"ID: {shipperReply.ShipperId}, Name: {shipperReply.CompanyName}, Phone: {shipperReply.Phone}.";
        }
    ...
    {
        EmployeesReply reply = await _employeeClient.GetEmployeesAsync(new EmployeesRequest());
